  ...
  return (
    <>
      ...
      <section style={{ background: R.bg, padding: "0" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: px, display: "grid", gridTemplateColumns: m ? "1fr" : "240px 1fr", gap: 0 }}>
          ...
          <div style={{ padding: m ? "40px 0 56px" : "48px 0 80px 48px" }}>
            {SERVICES.map((svc, i) => {
              const accent = i % 2 === 0 ? R.teal : R.gold;
              return (
                <div key={svc.num} id={`svc-${svc.num}`} style={{ scrollMarginTop: 100, marginBottom: i < SERVICES.length - 1 ? (m ? 40 : 56) : 0, paddingBottom: i < SERVICES.length - 1 ? (m ? 40 : 56) : 0, borderBottom: i < SERVICES.length - 1 ? `1px solid ${R.border}` : "none" }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 14, marginBottom: 10 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: accent }}>{svc.num}</span>
                    <h2 style={{ fontSize: m ? 20 : 26, fontWeight: 700, color: R.accent, margin: 0 }}>{svc.title}</h2>
                  </div>
                  ...
                  <div style={{ background: R.card, border: `1px solid ${R.border}`, borderRadius: 10, padding: m ? "16px 18px" : "20px 22px", display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr 1fr", gap: 8 }}>
